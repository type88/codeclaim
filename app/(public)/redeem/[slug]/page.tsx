...
export default function RedeemPage() {
  ...
  const [redeemedBundle, setRedeemedBundle] = useState<{ platform: string; code_value: string }[] | null>(null);
  ...
  const [enableBundles, setEnableBundles] = useState(false);
  ...
  useEffect(() => {
    ...
    async function fetchProjectInfo() {
      try {
        ...
        setSocialProof(data.data.social_proof || null);
        ...
        if (data.data.detected_platform && data.data.has_codes_for_detected) {
          if (data.data.enable_bundles) {
            setSelectedPlatforms(new Set([data.data.detected_platform]));
          } else {
            setSelectedPlatform(data.data.detected_platform);
          }
        }

        if (data.data.require_auth && !data.data.expired) {
  ...
  const handleBundleRedeem = useCallback(async () => {
    if (selectedPlatforms.size === 0) return;
    setRedeeming(true);
    setError(null);

    try {
      const response = await fetch(`/api/redeem/${slug}/bundle`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!data.success) {
        setError(data.error || "Failed to get codes");
        setRedeeming(false);
        return;
      }

      setRedeemedBundle(data.data.codes);
      setRedeeming(false);
    } catch (err) {
      if (err instanceof DOMException && err.name === "AbortError") return;
      setError("Network error. Please check your connection and try again.");
  ...
  const copyBundleCode = async (code: string, index: number) => {
    try {
  ...
  const socialProofText = useMemo(() => {
    if (!project?.show_social_proof || !socialProof) return null;
    if (project.social_proof_style === "remaining") {
      return `${socialProof.available_codes} codes remaining`;
    }
    return `${socialProof.used_codes} codes claimed`;
  }, [project, socialProof]);

  // Countdown for expiring links
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      ...
      if (days > 0) setCountdown(`${days}d ${hrs}h remaining`);
      else if (hrs > 0) setCountdown(`${hrs}h ${mins}m remaining`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/" className="font-medium hover:underline" style={{ color: accentColor }}>
            Go to homepage
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {project?.developer_logo_url && (
              <img src={project.developer_logo_url} alt="Logo" className="w-8 h-8 rounded-md object-cover" />
            )}
            <Link href="/" className="text-xl font-bold" style={{ color: accentColor }}>
              SudoGrab
            </Link>
          </div>
          {countdown && (
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            {!project?.hero_image_url && project?.icon_url && (
              <img src={project.icon_url} alt={project.name} className="w-20 h-20 mx-auto mb-4 rounded-xl" />
            )}
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {project?.promo_headline || project?.name}
            </h1>
            {project?.promo_headline && (
              <p className="mt-1 text-sm text-gray-500">{project.name}</p>
            )}
            {project?.promo_description ? (
              <div className="mt-3 text-gray-600 dark:text-gray-400 prose prose-sm dark:prose-invert max-w-none text-left">
                <ReactMarkdown>{project.promo_description}</ReactMarkdown>
              </div>
            ) : project?.description ? (
            ...
            {/* Social Proof */}
            {socialProofText && !expired && (
              <div className="mt-4 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
          ...
          {expired ? (
            <div className="text-center py-8">
              ...
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Campaign Ended</h2>
              <p className="text-gray-600 dark:text-gray-400">
                This code distribution has expired. Check back for future campaigns!
              </p>
            </div>
          ) : needsAuth ? (
            /* Auth Gate */
            ...
            /* Bundle Success */
            <div className="rounded-lg p-6 text-center" style={{ backgroundColor: `${accentColor}10` }}>
              <div className="mb-4" style={{ color: accentColor }}>
                <svg className="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Your Codes</h2>
              <div className="space-y-3 mb-4">
                {redeemedBundle.map((item, index) => (
                  <div key={index} className="bg-white dark:bg-gray-800 rounded-lg p-4 text-left">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-gray-500 flex items-center gap-1.5">
                        <span>{platformIcons[item.platform as PlatformType] || ""}</span>
                        {platformLabels[item.platform as PlatformType] || item.platform}
                      </span>
                      <button
                        onClick={() => copyBundleCode(item.code_value, index)}
                        className="text-xs px-2 py-1 rounded hover:opacity-80 text-white"
            ...
            <div className="rounded-lg p-6 text-center" style={{ backgroundColor: `${accentColor}10` }}>
              ...
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Your Code</h2>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2 text-white rounded-md hover:opacity-90"
                  style={{ backgroundColor: accentColor }}
                >
                  {copied ? (
                    <>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      {enableBundles ? "Select your platforms" : "Select your platform"}
                    </label>
                    {enableBundles && (
                      <p className="text-xs text-gray-500 mb-2">Choose multiple platforms to get one code for each.</p>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      {availablePlatforms.map(([platform, info]) => {
                        const isSelected = enableBundles
                            ...
                            <div className="flex items-center gap-2">
                              {enableBundles && (
                                <span className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${
                                  isSelected ? "border-transparent text-white" : "border-gray-400"
                              ...
                              <div>
                                ...
                                <div className="text-xs text-gray-500">{info.count} available</div>
                              </div>
                            </div>
                            {platform === detectedPlatform && (
                              <div className="mt-2 text-xs" style={{ color: accentColor }}>Detected</div>
                            )}
                          </button>
                        );
                      })}
                  ...
                  <button
                    onClick={enableBundles ? handleBundleRedeem : handleRedeem}
                    disabled={enableBundles ? selectedPlatforms.size === 0 || redeeming : !selectedPlatform || redeeming}
                    className="w-full py-3 px-4 text-white font-medium rounded-md hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{ backgroundColor: accentColor }}
                  >
                    {redeeming
                      ? enableBundles ? "Getting your codes..." : "Getting your code..."
                      : enableBundles
                        ? (selectedPlatforms.size > 1 ? `Get ${selectedPlatforms.size} Codes` : (project?.cta_text || "Get My Code"))
                        : (project?.cta_text || "Get My Code")}
                  </button>
                </div>
              ) : (
                <div className="text-center py-8">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No Codes Available</h2>
                  <p className="text-gray-600 dark:text-gray-400">
      ...
      <footer className="border-t border-gray-200 dark:border-gray-800 py-4">
        <div className="max-w-3xl mx-auto px-4 text-center text-sm text-gray-500">
          Powered by{" "}
          <Link href="/" className="hover:underline" style={{ color: accentColor }}>
            SudoGrab
          </Link>
        </div>
      </footer>
